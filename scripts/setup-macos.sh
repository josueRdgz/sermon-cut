#!/usr/bin/env bash
# Sermon Cut / Sermon Clips — reproducible setup for macOS (no Docker).
set -euo pipefail
...
need_cmd python3 "Instala Python 3.12+: brew install python@3.12"
need_cmd node "Instala Node.js 18+: brew install node"
need_cmd npm "npm suele venir con Node.js (brew install node)."
need_cmd ffmpeg "Instala FFmpeg: brew install ffmpeg"
need_cmd ffprobe "FFprobe viene con FFmpeg: brew install ffmpeg"

PY_VER="$(python3 -c 'import sys; print("%d.%d"%sys.version_info[:2])')"
python3 -c 'import sys; raise SystemExit(0 if sys.version_info>=(3,12) else 1)' \
  || fail "Se requiere Python ≥ 3.12 (encontrado ${PY_VER})."
...
pip install -e ".[dev]"
# Optional extras are documented; whisper/gemini/tracking are heavy — skip by default.
